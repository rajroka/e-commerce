...
export default function ReviewSection({ productId }: { productId: string }) {
  ...
  const StarRow = ({ count, active }: { count: number; active: boolean }) => (
    <HugeiconsIcon icon={StarIcon} size={count} color={active ? '#facc15' : '#e5e7eb'} strokeWidth={STROKE} />
  );
          ...
          {reviews.length > 0 && (
            <div className="sm:ml-auto flex items-center gap-2">
              <div className="flex gap-0.5">
                {[1,2,3,4,5].map(s => (
                  <HugeiconsIcon key={s} icon={StarIcon} size={18} color={s <= Math.round(avgRating) ? '#facc15' : '#e5e7eb'} strokeWidth={STROKE} />
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  ...
                  <div className="flex gap-1">
                    {[1,2,3,4,5].map(s => (
                      <button key={s} type="button" onClick={() => setRating(s)}
                        ...
                        <HugeiconsIcon icon={StarIcon} size={24} color={s <= (hovered || rating) ? '#facc15' : '#e5e7eb'} strokeWidth={STROKE} />
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <div className="space-y-6">
            {loading ? (
              ...
              <p className="text-sm text-gray-500 text-center py-8 border border-dashed border-gray-200 rounded-xl">No reviews yet. Be the first!</p>
            ) : reviews.map(review => (
              <div key={review._id} className="border-b border-gray-100 pb-6 last:border-0">
                <div className="flex items-center gap-3 mb-3">
                  {review.userImage
                    ? <Image src={review.userImage} alt={review.userName} width={32} height={32} className="rounded-full" />
                    : <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs font-semibold text-gray-600">{review.userName.charAt(0).toUpperCase()}</div>}
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{review.userName}</p>
                    <div className="flex gap-0.5 mt-0.5">
                      {[1,2,3,4,5].map(s => (
                        <HugeiconsIcon key={s} icon={StarIcon} size={11} color={s <= review.rating ? '#facc15' : '#e5e7eb'} strokeWidth={STROKE} />
                      ))}
                    </div>
                  </div>
