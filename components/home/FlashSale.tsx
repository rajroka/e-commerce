...
import { StarIcon, ChevronLeftIcon, ChevronRightIcon } from '@hugeicons/core-free-icons';
...
function useCountdown(target: Date) {
  const calc = () => { const d = target.getTime() - Date.now(); if (d <= 0) return { h:0,m:0,s:0 }; return { h:Math.floor(d/3600000), m:Math.floor((d/60000)%60), s:Math.floor((d/1000)%60) }; };
  const [t, setT] = useState(calc);
  useEffect(() => { const id = setInterval(() => setT(calc()), 1000); return () => clearInterval(id); }, []); // eslint-disable-line
  return t;
}

const pad = (n: number) => String(n).padStart(2, '0');
const DISCOUNTS = [20, 30, 35, 25, 40, 50, 45, 30];
const salePrice = (price: number, pct: number) => (price * (1 - pct / 100)).toFixed(2);
...
export default function FlashSale() {
  ...
  useEffect(() => {
    axios.get('/api/products').then(r => setProducts((r.data.products || r.data).slice(0, 8))).catch(console.error).finally(() => setLoading(false));
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <Link href="/products" className="text-sm font-medium text-red-500 hover:underline hidden sm:block">See All →</Link>
              ...
              <div key={p._id || p.id} className="flex-shrink-0 w-52 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group flex flex-col overflow-hidden">
                ...
                <div className="p-3 flex flex-col gap-1 flex-1">
                  ...
                  <div className="flex items-center gap-0.5">
                    {[1,2,3,4,5].map(j => (
                      <HugeiconsIcon key={j} icon={StarIcon} size={10} color={j <= Math.round(p.rating ?? 4) ? '#facc15' : '#e5e7eb'} strokeWidth={STROKE} />
                    ))}
                  ...
                  <button onClick={() => handleCart(p)} className="mt-2 w-full bg-red-500 hover:bg-red-600 text-white text-xs font-semibold py-1.5 rounded-full transition-colors">Add to Cart</button>
                </div>
              </div>
            );
          })}
